...
for line in myList:
    ...
    #         characters['e'] = "abcdefg".translate(None, ''.join(list(numbers[5]))).replace('b',"")
    ...
    # Check for 1,4,7,8 in the output values
    counter += line[1].count(numbers[4])
    counter += line[1].count(numbers[1])
    counter += line[1].count(numbers[7])
    counter += line[1].count(numbers[8])
